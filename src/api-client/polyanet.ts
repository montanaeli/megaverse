...
export async function createPolyanet(row: number, column: number) {
  try {
    ...
    if (response) {
      console.log(`Polyanet created on row ${row} and column ${column}`);
    } else {
...
export async function deletePolyanet(row: number, column: number) {
  try {
    const response = await axios.delete(`polyanets/`, {
      data: { row, column },
    });
    if (response) {
      console.log(`Polyanet deleted on row ${row} and column ${column}`);
    } else {
